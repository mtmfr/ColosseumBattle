...
public abstract class Old_Hero : Old_Unit
{
    /// <summary>
    ...
    protected override void OnSearchClosestOpponent()
    {
        ...
        List<Old_Enemy> enemyList = FindObjectsByType<Old_Enemy>(FindObjectsInactive.Exclude, FindObjectsSortMode.None).ToList();

        opponentToTarget = enemyList.OrderBy(enemy => Vector3.Distance(enemy.transform.position, transform.position)).FirstOrDefault().gameObject;
    }
